const apiUrl = 'https://gladys.id/wp-json/myapi/v1/chat-bot/';
const botConfigurationUrl = 'https://gladys.id/wp-json/myapi/v1/chat-bot-config';
...
function lwhOpenCbotSendMessage(message) {
...
messageInput.addEventListener('keydown', function(event) {
    if (event.key === 'Enter' && !event.shiftKey) { // Prevent sending message on Shift + Enter
        event.preventDefault();
        lwhOpenCbotonFormSubmit(event);
    }
});

// Optionally, you can also listen for the send button click
...
function lwhOpenCbotaddMessage(sender, message) {
    ...
    // Ganti teks yang diapit oleh *...* menjadi <b> (bold)
    message = message.replace(/\*(.*?)\*/g, '<b>$1</b>');  // Ini untuk membuat teks tebal
    
    // Ganti teks yang diapit oleh _..._ menjadi <i> (italic)
    message = message.replace(/_(.*?)_/g, '<i>$1</i>');  // Ini untuk membuat teks miring
    
    // Ganti teks yang diapit oleh ~...~ menjadi <s> (strikethrough)
    message = message.replace(/~(.*?)~/g, '<s>$1</s>');  // Ini untuk membuat teks dicoret
...
function lwhOpenCbotreplaceTypingAnimationWithMessage(sender, message) {
    ...
    if (typingContainer) {
        ...
        // Menangani format * untuk teks tebal (bold)
        ...
        messageDiv.innerHTML = 
                ...
                : ""}
                <p class="typing-effect">${formattedMessage}</p>
                ${sender === 'user' ?
                    `<div>
                        <img width="30px" class="avatar-image"
                            src="${botConfigData.userAvatarURL}"
        ...
        const typingInterval = setInterval(() => {
            ...
            if (index === message.length) {
                ...
                typingEffectElement.insertAdjacentHTML('beforeend', `<span title="copy" class="copy-text" onclick="lwhOpenCbotcopyText(event)">...</span>`);
            }
        }, 10);
    }
}
